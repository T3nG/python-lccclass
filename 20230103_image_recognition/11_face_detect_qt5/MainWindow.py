...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def loadModelThreadCallback(self, data):
        ...
        self.picThread = PictureThread(self.defaultPath)
    ...
    def btnPictureClick(self, tag):
        ...
        distance = [np.linalg.norm(target - d) for d in self.descriptors]
        ...
        x1 = 0
        y1 = 0
        _, _, t_x2, t_y2 = draw.textbbox(xy=(x1, y1), text=name, font=font)
        x2 = t_x2
        y2 = t_y2
        draw.rectangle((x1, y1, x2, y2), fill=(150, 150, 255, 100))
        draw.text((x1, y1), name, font=font, fill=(0, 255, 0))
        img = np.asarray(pil)
